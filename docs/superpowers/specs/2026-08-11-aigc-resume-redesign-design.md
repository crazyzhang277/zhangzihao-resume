...
页面不再沿用现有的浅灰卡片和苹果玻璃拟态作为主语言。新的基调是纸张感浅灰背景、近黑正文、电光蓝和少量珊瑚橙强调色，配合大字号编辑型排版和严谨的工程元数据。

首屏右侧使用程序化的 AI 渲染实验场：光带、网格、粒子、景深和参数标签构成技术气质。它是抽象的品牌视觉，不冒充真实作品。移动端将其降级为静态海报式画面，保证文本和操作优先。
