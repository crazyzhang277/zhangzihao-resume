    ...
    <div className="skills-section">
      <div aria-label="Skill categories" className="skill-tabs" role="tablist">
        {groups.map((group, index) => (
          <button aria-controls={`skill-panel-${index}`} aria-selected={safeIndex === index} id={`skill-tab-${index}`} key={group.name} onClick={() => selectTab(index)} onKeyDown={(event) => onTabKeyDown(event, index)} ref={(node) => { tabRefs.current[index] = node }} role="tab" tabIndex={safeIndex === index ? 0 : -1} type="button">
            {group.name}
          </button>
        ))}
      </div>
      {groups.map((group, index) => (
        <div aria-labelledby={`skill-tab-${index}`} className="skill-panel" hidden={safeIndex !== index} id={`skill-panel-${index}`} key={group.name} role="tabpanel">
          <ul>
