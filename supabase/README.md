...
The sync uses GitHub pagination, excludes forks, archived repositories, `zeroaigen-auto-mention`, and `project_exclusions`, then upserts only GitHub-owned fields. It never sends manual visibility, featured rank, or manual title/description fields in its upsert payload. A failed GitHub fetch does not alter cached projects; after a successful fetch, projects not returned by the eligible result set become `stale` and `visible = false` rather than being deleted.
